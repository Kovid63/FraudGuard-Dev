...
function isPaymentMethodPaypal(transactionsData) {
  ...
  for (const transaction of transactionsData.transactions) {
    if (transaction.payment_details?.payment_method_name === "paypal")
      return true;
  }
  return false;
}
  ...
  const apis = [
    {
      ...
      parser: (data) => data?.Scheme || data?.Brand,
    },
    {
  ...
  if (result.isValid && !trustedProviders.has(domain)) {
    ...
    if (
      !result.issues.includes(
        "Non-trusted domain (only popular email providers allowed)"
      )
    ) {
      result.issues.push(
        "Non-trusted domain (only popular email providers allowed)"
  ...
  function checkSuspiciousDomainPatterns(domain, result) {
    ...
    for (const pattern of suspiciousPatterns) {
      if (pattern.test(domainWithoutTld)) {
        ...
        if (
          /support|help|service|customer|billing|account|admin/i.test(
            domainWithoutTld
          )
        ) {
          result.issues.push(
            "Domain contains service-related keywords (possible impersonation)"
          );
        } else {
...
async function fetchCountryStatistics(shopifyClient) {
  ...
  try {
    ...
    console.log(
      `Analyzed ${totalOrders} orders from ${
  ...
  try {
    ...
    while (hasNextPage) {
      ...
      if (response?.data?.orders?.edges) {
        ...
        console.log(
          `Fetched ${allOrders.length} orders for AOV/CPA analysis...`
        );
    ...
    const orderValues = allOrders
      .map(({ node }) =>
        parseFloat(node.totalPriceSet?.shopMoney?.amount || "0")
      )
      .filter((value) => value > 0);

    const totalRevenue = orderValues.reduce((sum, value) => sum + value, 0);
    const averageOrderValue =
      orderValues.length > 0 ? totalRevenue / orderValues.length : 0;
    ...
    allOrders
      ...
      .forEach(({ node }) => {
        ...
        if (
          customerId &&
          orderValue > 0 &&
          !customerFirstPurchases.has(customerId)
        ) {
    ...
    const totalFirstPurchaseRevenue = firstPurchaseValues.reduce(
      (sum, value) => sum + value,
      0
    );
    const averageCPA =
      firstPurchaseValues.length > 0
        ? totalFirstPurchaseRevenue / firstPurchaseValues.length
        : averageOrderValue;

    console.log(
      `AOV/CPA Analysis: AOV $${averageOrderValue.toFixed(
        2
      )}, CPA $${averageCPA.toFixed(2)} from ${allOrders.length} orders`
    );
...
async function checkLowVolumeCountry(
  shopifyClient,
  orderData,
  cacheKey = "country_stats"
) {
  try {
    ...
    const countryPercentage =
      totalOrders > 0 ? (countryOrderCount / totalOrders) * 100 : 0;
    const isLowVolumeCountry = countryPercentage < 5;

    console.log(
      `Country ${orderCountryCode}: ${countryOrderCount}/${totalOrders} orders (${countryPercentage.toFixed(
        2
      )}%)`
    );
    ...
    console.error("Error checking low volume country:", error.message, {
      category: "country-analysis",
    });
    return {
      isLowVolumeCountry: false,
      countryPercentage: 0,
      error: error.message,
    };
  }
    ...
    if (customerId) {
      ...
      if (response?.data?.customer?.orders?.edges) {
        ...
        const previousOrders = orders.filter(
          ({ node }) => node.id !== orderData.admin_graphql_api_id
        );
...
async function checkFirstTimeHighAOV(
  shopifyClient,
  orderData,
  cacheKey = "aov_cpa_stats"
) {
  try {
    const isFirstTimeCustomer = await checkIfFirstTimeCustomer(
      shopifyClient,
      orderData
    );
    ...
    const currentOrderValue = parseFloat(
      orderData.current_total_price || orderData.total_price || "0"
    );
    ...
    console.log(
      `First-time customer AOV analysis: Order $${currentOrderValue} vs 3x CPA threshold $${thresholdAmount.toFixed(
        2
      )}`
    );
    ...
    console.error("Error checking first-time high AOV:", error.message, {
...
function calculateFinalScore(baseScore, trustDeductions) {
  const cappedDeductions = Math.max(
    trustDeductions,
    SCORING_LIMITS.MAX_POSITIVE_DEDUCTION
  );
...
export const getRiskLevel = async (
  ...
  try {
    ...
    return {
      ...
      risk: "unknown",
      recommendation:
        "Unable to assess risk due to technical error. Manual review recommended.",
      error: error.message,
    };
  }
  ...
  if (db && orderTxnDetails && orderTxnDetails.length > 0 && cardBrand) {
    // Extract LAST4 from successful transactions
    const last4List = orderTxnDetails
      .filter((txn) => !!txn.accountNumber && txn.status === "SUCCESS")
      .map((txn) => txn.accountNumber.replace(/\D/g, "").slice(-4));

    if (last4List.length > 0) {
      try {
        const wasFlaggedBefore = await db.collection("orders").findOne({
          shop,
          // Match last4 instead of full card number
          "guard.txnDetails.last4": { $in: last4List },
          "guard.cardBrand": cardBrand,
          // Match only suspicious past orders
          "guard.risk": { $in: ["high", "high-medium", "medium"] },
        });
        ...
        if (hasBeenUsedBefore) {
          ...
          console.warn(
            `Card last4 reused with same brand (${cardBrand}) in a past suspicious order`
          );

          baseScore += 37; // NEW SCORE
          reason.push(
            `Card used in past suspicious order – Partial match (${cardBrand} ****${last4})`
          );
        }
      } catch (error) {
        console.error(
          "Error checking past suspicious card use:",
          error.message
        );
        reason.push("Error checking past suspicious card use");
      }
    }
  }
  ...
  try {
    const countryRiskAnalysis = await checkLowVolumeCountry(
      shopifyClient,
      order,
      `country_stats_${shop}`
    );
    if (countryRiskAnalysis?.isLowVolumeCountry) {
      baseScore += 6;
      reason.push(
        `Order from ${countryRiskAnalysis.countryCode} - only ${countryRiskAnalysis.countryPercentage}% of total orders`
      );
      console.log(
        `Low volume country risk: ${countryRiskAnalysis.countryCode} (${countryRiskAnalysis.countryPercentage}%)`
      );
    } else if (countryRiskAnalysis?.countryPercentage > 20) {
      trust.push(
        `Order from a popular country (${countryRiskAnalysis.countryCode}: ${countryRiskAnalysis.countryPercentage}%)`
      );
    }
  } catch (error) {
  ...
  // AOV Risk Analysis
  try {
    const aovRiskAnalysis = await checkFirstTimeHighAOV(
      shopifyClient,
      order,
      `aov_cpa_stats_${shop}`
    );
    if (aovRiskAnalysis?.isFirstTimeHighAOV) {
      baseScore += 9;
      reason.push(
        `First-time customer with ${aovRiskAnalysis.multiplier}x average CPA ($${aovRiskAnalysis.currentOrderValue} vs $${aovRiskAnalysis.thresholdAmount} threshold)`
      );
      console.log(
        `High AOV first-time customer risk: ${aovRiskAnalysis.multiplier}x CPA`
      );
    }
  } catch (error) {
  ...
  // Orders Today Risk Analysis
  try {
    const customerEmail = order?.email || order?.customer?.email;

    if (customerEmail) {
      console.log(`\n🟦 Checking orders today for: ${customerEmail}`);

      const startOfDay = new Date();
      startOfDay.setHours(0, 0, 0, 0);
      console.log(`Start of day boundary: ${startOfDay.toISOString()}`);

      const ordersTodayQuery = `
      query getOrdersToday($query: String!, $first: Int!) {
        orders(first: $first, query: $query) {
          edges {
            node {
              id
    ...
    `;
      ...
      const todayQueryString = `email:"${customerEmail}" created_at:>=${startOfDay.toISOString()}`;
      console.log(`Shopify Query: ${todayQueryString}`);

      const ordersTodayResponse = await shopifyClient.request(
        ordersTodayQuery,
        {
          variables: {
            query: todayQueryString,
            first: 50,
          },
        }
      );

      const ordersToday = ordersTodayResponse?.data?.orders?.edges || [];

      console.log(`Total orders returned for today: ${ordersToday.length}`);

      // Exclude current order safely
      const previousOrdersToday = ordersToday.filter(({ node }) => {
        const nodeId = node.id.toString().replace(/\D/g, "");
        const currentId = order.id.toString().replace(/\D/g, "");
        return nodeId !== currentId; // Exclude the current order correctly
      });

      console.log(
        `Previous orders found today (excluding current): ${previousOrdersToday.length}`
      );

      previousOrdersToday.forEach((o, index) => {
        console.log(
          `  -> Order #${index + 1}: ID ${o.node.id}, createdAt ${
            o.node.createdAt
          }`
        );
      });

      const count = previousOrdersToday.length;

      if (count === 1) {
        baseScore += 5;
        console.log(" Applied: SECOND order today (+5)");
        reason.push("Second order today (+5)");
      } else if (count >= 2) {
        baseScore += 10;
        console.log(" Applied: THIRD or more order today (+10)");
        reason.push("Third or more order today (+10)");
      } else {
        console.log(" First order today — no score increase");
      }
    } else {
      console.log(" No customer email found — skipping orders-today check.");
    }
  } catch (error) {
    console.error(
      " Error checking orders-today risk indicator:",
      error.message
    );
    reason.push("Error checking orders-today indicator");
  }
  ...
  try {
    ...
    if (
      transactionData?.transactions?.length === 1 &&
    ...
    console.error(
      "Error fetching or analyzing transaction data:",
      error.message
    );
  ...
  if (finalScore >= 85) {
    risk = "high";
    recommendation =
      "URGENT - Cancel this order immediately. Based on multiple high-risk signals, verification is not recommended.";
  } else if (finalScore >= 70) {
    risk = "high-medium";
    recommendation =
      "Verification required. If the customer fails to verify, we recommend cancellation. If the customer successfully verifies, approve the order.";
  } else if (finalScore >= 55) {
    risk = "medium";
    recommendation =
      "Verification recommended. If verification fails or is not completed, cancel the order. If verified, approve.";
  } else if (finalScore >= 40) {
    risk = "low-medium";
    recommendation =
      "If no multiple credit cards use are detected and some positive trust signals exist - you may approve the order. Optionally, wait for verification to be more certain.";
  } else {
    risk = "low";
    recommendation =
      "No signs of fraud detected. If there is no past record of fraudulent behavior, we recommend approval.";
  }
  ...
  if (
    shopifyRiskAssessments?.assessments?.[0]?.riskLevel === "MEDIUM" &&
  ...
  console.log(`Risk Assessment Summary:
        ...
        Trust Deductions: ${trustDeductions} (capped at ${
    SCORING_LIMITS.MAX_POSITIVE_DEDUCTION
  })
        Final Score: ${finalScore < 1 ? 1 : finalScore}
        Risk Level: ${risk}
        High Risk Floor Applied: ${
          baseScore >= SCORING_LIMITS.HIGH_RISK_THRESHOLD ? "Yes" : "No"
        }
    `);
...
async function processOrderWithAOVCheck(orderData, session) {
  ...
  if (aovAnalysis.isFirstTimeHighAOV) {
    console.log(
      `High AOV first-time customer: ${aovAnalysis.currentOrderValue} (${aovAnalysis.multiplier}x average CPA)`
    );
  }
