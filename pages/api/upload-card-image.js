  ...
  try {
    ...
    try {
      ...
      const { url } = await put(blobPath, fileStream, {
        ...
        // Set TTL for automatic deletion after 90 days
        cacheControlMaxAge: 90 * 24 * 60 * 60, // 90 days in seconds
        expires: ttlDate.toISOString(), // ISO string for expiration
      });
      
      console.log(`Image uploaded with TTL: ${ttlDate.toISOString()}`);
