...
// Exported function to get BIN from order ID
export async function getBinFromOrderId(shop, orderId, session) {
  ...
  return {
    bin: txnDetails.paymentDetails.bin,
    lastFourDigits: txnDetails.paymentDetails.number.replace(/[^0-9]/g, '').slice(-4)
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Use the extracted function to get BIN and payment details
    const { bin, lastFourDigits: creditCardLastFour } = await getBinFromOrderId(shop, orderId, session);

    // Get card BIN country and validate
    const binCountry = await getIssuingCountryFromBin(bin);
